...
hold on
semilogy(SNR_Range,MSE_HA02_over_SNR_Two_path, 'Marker', 'v', 'LineWidth', 1, 'LineStyle', '-.');
hold on
...
legend('LS estimate tested on the CE channel', ...
    ...
    'Channelformer tested on the CE channel', ...
...
xlabel('SNR in dB');
ylabel('MSE');
title('Generalization of Channelformer to fixed PDP channels');
grid on;
hold off;
